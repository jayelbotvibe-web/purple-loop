...
.PHONY: help host-prep lab-fetch atomics lab-up lab-down archives verify run build vet reset snapshot restore
...
atomics: ## vendor Atomic Red Team at the pinned commit (required before any run)
	@bash scripts/fetch-atomics.sh

lab-up: ## bring the merged Wazuh stack + victim up
	@cd $(LAB_DIR) && docker compose up -d
	@echo "give the indexer ~60-90s, then: make archives && make verify"

archives: ## enable Wazuh archive logging (the collector reads archives.json)
	@bash scripts/enable-archives.sh
...
build: ## compile everything (scoped: the vendored ART tree is not Go source)
